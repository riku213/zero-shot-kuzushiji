...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a multi-scale patch CNN FaRE recognition model from the separated kana/character dataset.")
    parser.add_argument(
        "--data-root",
    ...
    parser.add_argument(
        ...
        type=int,
        default=[2, 4, 8],
        help="Patch grid scales for the multi-scale branch (e.g. 2 4 8).",
    )
    parser.add_argument(
        "--patch-image-size",
        type=int,
        default=24,
        help="Per-patch resize target fed into the shared CNN encoder.",
    )
    parser.add_argument(
        "--encoder-dim",
        type=int,
        default=256,
        help="Output feature dimension of the shared patch CNN encoder.",
    )
    parser.add_argument(
        "--fusion-hidden-dim",
        type=int,
        default=512,
        help="Hidden dimension of the fusion MLP that maps multi-scale features to codebook logits.",
    )
    parser.add_argument(
        "--dropout",
        type=float,
        default=0.2,
        help="Dropout ratio in the fusion MLP.",
    )
...
class FareRecognitionModel(nn.Module):
    def __init__(
        self,
        codebook_dim: int = 1212,
        patch_scales: tuple[int, ...] = (2, 4, 8),
        patch_image_size: int = 24,
        encoder_dim: int = 256,
        ...
        super().__init__()
        if not patch_scales:
            raise ValueError("patch_scales must contain at least one scale.")
        if any(scale <= 0 for scale in patch_scales):
            raise ValueError("patch_scales must contain positive integers.")
        if patch_image_size <= 0:
            raise ValueError("patch_image_size must be positive.")

        self.patch_scales = tuple(int(scale) for scale in patch_scales)
        self.patch_image_size = int(patch_image_size)

        # Shared patch encoder for all scales (2x2 / 4x4 / 8x8 branches).
        self.patch_encoder = nn.Sequential(
            ...
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            nn.Conv2d(128, encoder_dim, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(encoder_dim),
            nn.ReLU(inplace=True),
            nn.AdaptiveAvgPool2d((1, 1)),
        )

        fusion_input_dim = encoder_dim * len(self.patch_scales)
        self.fusion = nn.Sequential(
            nn.Linear(fusion_input_dim, fusion_hidden_dim),
            nn.ReLU(inplace=True),
            nn.Dropout(p=dropout),
            nn.Linear(fusion_hidden_dim, codebook_dim),
        )

    def _extract_patches(self, x: torch.Tensor, grid_size: int) -> torch.Tensor:
        bsz, channels, height, width = x.shape
        patch_h = max(1, height // grid_size)
        patch_w = max(1, width // grid_size)
        crop_h = patch_h * grid_size
        crop_w = patch_w * grid_size

        x = x[:, :, :crop_h, :crop_w]
        x = x.view(bsz, channels, grid_size, patch_h, grid_size, patch_w)
        x = x.permute(0, 2, 4, 1, 3, 5).contiguous()
        patches = x.view(bsz * grid_size * grid_size, channels, patch_h, patch_w)
        if patch_h != self.patch_image_size or patch_w != self.patch_image_size:
            patches = F.interpolate(
                patches,
                size=(self.patch_image_size, self.patch_image_size),
                mode="bilinear",
                align_corners=False,
            )
        return patches
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for scale in self.patch_scales:
            patches = self._extract_patches(x, scale)
            encoded = self.patch_encoder(patches).flatten(1)
            encoded = encoded.view(bsz, scale * scale, -1)
            aggregated = encoded.mean(dim=1)
            multi_scale_features.append(aggregated)
...
def train_model_on_dataset(
    ...
    train_dataset = CharacterImageDataset(train_entries, image_size=image_size)
    val_dataset = CharacterImageDataset(val_entries, image_size=image_size)
    num_workers = 0
...
def main() -> None:
    ...
    try:
        ...
        print(
            "Model config: "
            f"type=MultiScalePatchCNN scales={args.patch_scales} patch_image_size={args.patch_image_size} "
            f"encoder_dim={args.encoder_dim} fusion_hidden_dim={args.fusion_hidden_dim} dropout={args.dropout}"
        )
        if log_path is not None:
        ...
        if args.pretrain_root:
            ...
            if pretrain_train_entries or pretrain_val_entries:
                ...
                pretrain_model = FareRecognitionModel(
                    codebook_dim=pretrain_codebook_matrix.shape[1],
        ...
        metadata = {
            ...
            "seed": args.seed,
            "device": args.device,
            "model_type": "multiscale_patch_cnn",
            "model_config": {
